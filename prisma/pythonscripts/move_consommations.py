...
try:
    ...
    if connection.is_connected():
        ...
        for i in range(100):
            ...
            cursor.execute("SELECT * FROM consommations_old")
            consos = {row[0]:row for row in cursor.fetchall()}

            cursor.execute("SELECT * FROM historique_fams")
            transaction = {row[0]:row for row in cursor.fetchall()}
