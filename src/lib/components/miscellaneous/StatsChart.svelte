<script lang="ts">
    import Chart, { type ActiveElement, type ChartData, type ChartEvent, type ChartTypeRegistry, type TooltipItem, type TooltipModel } from 'chart.js/auto';
    ...
    const getBoqName = (id_boq:number|null) => boquettes.find(e=>e.id_boquette==id_boq)?.nom;
    onMount(() => chart = createChart());

    function createChart():Chart<"pie">{
    const labels = consos.map(e=>{
      const a=getBoqName(e.id_boquette)
      return a == undefined ? 'transfert':a
    });
    ...
    return new Chart(ctx,{
      ...
      options:{
        onClick: (event:ChartEvent, elements:ActiveElement[], chart:Chart) => {
            ...
            var elements = chart.getElementsAtEventForMode(event as any as MouseEvent, 'index', { intersect: true }, false);
