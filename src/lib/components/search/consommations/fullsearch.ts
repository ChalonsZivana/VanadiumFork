...
import type {
  ConsommationsSchemaType,
  PgSearchSchemaType,
} from "$lib/zodSchema";
import { Prisma, consommations_type } from "@prisma/client";

export async function consommationsSearch(
  types:
    | (
  ...
  const whereProms: { proms: number } | {} = data.proms
    ? { proms: data.proms }
    : {};
  const wherePg: Prisma.consommationsWhereInput = {
