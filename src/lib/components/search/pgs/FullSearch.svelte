...
<SectionCard {title}>
  ...
  >
    <div class="text-black w-full flex flex-col gap-2">
      <p class="text-white">
        Total de {totalPgs} pg(s) sur {nombrePages} page(s). (20 par page)
      </p>
      ...
      <button type="submit">
        {#if page > 1}
          <button
            type="button"
            on:click={() => (page -= 1)}
            class="bg-white p-2 h-10 rounded-lg">précédent</button
          >
          <button
            type="button"
            on:click={() => (page = 1)}
            class="bg-white p-2 size-10 rounded-lg">1</button
          >
        {/if}
        <button type="button" class="bg-red-500 p-2 w-10 rounded-lg"
          >{page}</button
        >
        {#if page < nombrePages}
          <button
            ...
            class="bg-white p-2 size-10 rounded-lg">{nombrePages}</button
          >
          <button
            type="button"
            on:click={() => (page += 1)}
            class="bg-white p-2 h-10 rounded-lg">suivant</button
          >
        {/if}
      </button>
    </div>
      ...
      <svelte:fragment slot="tbody" let:e>
        {#key e}
          <tr
            ...
            class="divide-x-2 cursor-pointer divide-white"
          >
