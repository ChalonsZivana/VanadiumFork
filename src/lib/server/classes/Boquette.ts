...
export class Boquette extends HasMoney {
  ...
  async boquette() {
    return (await prisma.boquettes.findFirst({
  ...
  static async exists(id_boquette: number) {
    return (
      (await prisma.boquettes.findFirst({ where: { id_boquette } })) != null
    );
  }
  ...
  async editBoquette(data: z.infer<typeof EditBoquetteSchema>) {
    if (data.password == "") {
      await prisma.boquettes.updateMany({
        ...
        data: {
          nom: data.nom,
          partie_pg: data.partie_pg,
          nom_simple: data.nom_simple,
        },
      });
    } else {
