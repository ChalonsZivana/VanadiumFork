...
interface pg_ext
  extends Pick<consommations, "type" | "from" | "montant" | "libelle"> {
  type: "pg_ext";
  from: number;
}
interface ext_
  extends Pick<consommations, "type" | "to" | "montant" | "libelle"> {
  type: "ext_fams" | "ext_boq";
  to: number;
}
interface _to
  extends Pick<consommations, "type" | "from" | "to" | "montant" | "libelle"> {
  type: "pg_pg" | "pg_fams";
  from: number;
  to: number;
}
interface pg_boq
  extends Pick<
    consommations,
    "type" | "from" | "to" | "id_produit" | "quantite" | "rhopseur"
  > {
  type: "pg_boq";
  from: number;
  to: number;
...
async function getLibelle(d: ext_ | pg_ext | _to | pg_boq) {
  switch (d.type) {
    ...
    case "pg_boq":
      const prod = await prisma.produits.findFirst({
        where: { id_produit: d.id_produit },
      });
      if (prod == null) return null;

      return (
        `Consommation: ${d.quantite} * ${prod.nom}` +
        (d.rhopse_ancien ? ` (${d.rhopse_ancien})` : "")
      );
    default:
      return d.libelle;
  }
...
export class Taferie {
  static async cancelConsommation(
    id_conso: number,
    cancel: boolean,
  ): Promise<{ success: boolean; message: string } | consommations> {
    const conso = await prisma.consommations.findFirst({
      where: { id_conso: id_conso },
    });

    if (!conso || conso.annule == cancel)
    ...
    try {
      return await prisma.$transaction(async (p) => {
        switch (conso.type) {
          ...
          case "pg_boq":
            if (conso.id_produit == null || conso.quantite == null)
              throw new Error("old consommation");

            const prod = await prisma.produits.findFirst({
              where: { id_produit: conso.id_produit },
            });

            if (prod == null || prod?.id_boquette != conso.to)
              throw new Error("old consommation");
            if (prod.inventaire != null) {
              await p.produits.update({
                where: { id_produit: prod.id_produit },
                data: { inventaire: prod.inventaire + conso.quantite },
              });
            }
            if (conso.to == BOQUETTES["Foy's"]) {
              const data = conso.data as { prix: number; prix2: number } | null;
              if (data == null || !("prix" in data) || !("prix2" in data))
                throw new Error("old consommation");

              await new Boquette(BOQUETTES["Foy's"]).addMoney(
                data.prix * factor,
                p,
              ); //prix : soft
              await new Boquette(BOQUETTES["Satan"]).addMoney(
                data.prix2 * factor,
                p,
              ); //prix 2:hard

              new Pg(conso.from!).removeMoney(
                (data.prix + data.prix2) * factor,
                p,
              );
            } else {
              new Boquette(conso.to!).addMoney(conso.montant * factor, p);
          ...
          case "pg_pg":
            new Pg(conso.from!).removeMoney(conso.montant * factor, p);
            const consoBis = await p.consommations.findFirst({
              where: { id_conso: conso.to! },
            });
            if (
              !consoBis ||
              consoBis.annule == true ||
              conso.libelle != consoBis.libelle
            )
              throw new Error();
            new Pg(consoBis.from!).addMoney(conso.montant * factor, p);

            await p.consommations.update({
              where: { id_conso: consoBis.id_conso },
              data: { annule: cancel },
            });
            break;
        }
  ...
  static async rhopse(
    ...
    if (Math.abs(montant) > 100_000)
      return { success: false, message: `Montant trop élevé` };
      ...
      const transaction = await prisma.$transaction(async (p) => {
        ...
        switch (d.type) {
          case "pg_pg": {
            ...
            await p.consommations.update({
              where: { id_conso: conso.id_conso },
              data: { to: conso2.id_conso },
            });
            await p.consommations.update({
              where: { id_conso: conso2.id_conso },
              data: { to: conso.id_conso, solde_apres: pg2_after.solde },
            });
            break;
          }
          case "ext_boq": {
            const boq_after = await new Boquette(d.to).removeMoney(
              data.montant,
              p,
            );
          ...
          case "ext_fams": {
            const fams_after = await new Fams(d.to).removeMoney(
              data.montant,
              p,
            );
          ...
          case "pg_boq": {
            ...
            if (prod.inventaire != null) {
              await p.produits.update({
                where: { id_produit: prod.id_produit },
                data: { inventaire: prod.inventaire - d.quantite },
              });
            }
            // specific foys
            if (d.to == BOQUETTES["Foy's"]) {
              await new Boquette(BOQUETTES["Foy's"]).removeMoney(
                -prod.prix * d.quantite,
                p,
              );
              await new Boquette(BOQUETTES["Satan"]).removeMoney(
                -prod.prix2 * d.quantite,
                p,
              );
              ...
              await p.consommations.update({
                ...
                data: {
                  data: {
                    prix: -prod.prix * d.quantite,
                    prix2: -prod.prix2 * d.quantite,
                  },
                },
              });
            } else {
  ...
  static async consommations(
    types: (
      | { type: consommations_type; from: number }
      | { type: consommations_type; to: number }
    )[],
  ) {
...
export type ConsommationsIncludeType = Awaited<
  ReturnType<typeof Taferie.consommations>
>[0];
