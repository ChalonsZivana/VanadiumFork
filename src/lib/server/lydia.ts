...
import {
  LydiaDemandResponseSchema,
  LydiaVerifyResponseSchema,
} from "$lib/zodSchema";
import type { Prisma } from "@prisma/client";
import { Taferie } from "./classes/Taferie";

export class LydiaManager {
  static async createLydiaDemand(
    id_pg: number,
    varData: { montant: number; numero: string },
    vendorKey: string,
  ) {
    const _min = 10;
    const _max = 100.89;
    if (varData.montant < _min || varData.montant > _max)
      return {
        success: false,
        message: `Il faut un montant compris ${_min}€ et ${_max}€`,
      };
    ...
    const test = await prisma.rechargements.findMany({
      where: { status: 0, id_pg },
    });
    if (test.length != 0) {
    ...
    try {
      ...
      if (!lydiaData.success || lydiaData.data.error != "0")
        return { error: "Mauvais numéro de téléphone" };
      ...
      return {
        success: true,
        message: "Demande envoyée, veuillez l'accepter dans les 120s",
      };
    } catch (error) {
    ...
    try {
      ...
      if (lydiaData.data.state == "1") {
        ...
        await Taferie.rhopse({
          type: "pg_ext",
          from: d.id_pg,
          libelle: "Rechargement Lydia",
          montant: d.montant,
        });
        return {
          success: true,
          message: `${d.montant}€ ont été ajouté à votre sole.`,
        };
      } else {
        await prisma.rechargements.update({
          where: { id_rechargement, status: 0 },
          data: { status: -1 },
        });
