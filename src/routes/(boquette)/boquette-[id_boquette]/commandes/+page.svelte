<script lang="ts">
  ...
  async function updateData() {
    ...
    const new_commandes = [...commandesNonTraitées, ...result.commandes].sort(
...
<div
  class="h-full w-full flex flex-col justify-start items-start p-2 gap-4 {commandesNonTraitées.length
    ? ''
    : ''}"
>
  <Accordion autocollapse={true}>
    ...
    <AccordionItem
      open
      class={commandesNonTraitées.length > 0 ? "bg-red-600" : ""}
    >
      ...
      <svelte:fragment slot="summary">
        <p class="text-xl">
          Commandes non traitées - {commandesNonTraitées.length}
        </p>
      </svelte:fragment>
...
<SubmitDialog bind:dialog title="< Rhopser >">
