...
>
  <div class="grid grid-cols-5 w-11/12 place-items-center variant-filled-surface">
    {#each week as day}
      <div>
        <p>{day.dayName}</p>
      </div>
  ...
  <div class="relative w-11/12 h-5/6">
    <div class="absolute grid grid-cols-5 grid-rows-12 variant-filled-surface w-full h-full">
      {#each Array(5 * 12).fill(10) as t}
        <div class="border-1 border-gray-500"></div>
      {/each}
    </div>
    <div class="absolute grid grid-cols-5 grid-rows-12 bg-red-200 opacity-50 w-full h-full">
      {#if events}
