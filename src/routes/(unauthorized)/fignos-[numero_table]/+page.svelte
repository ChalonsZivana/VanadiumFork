<script lang="ts">
  ...
  const quantités = Object.fromEntries(
    data.produits.map((e) => [e.id_produit, 0]),
  );
  let categorie = data.categories[0];
  $: products = categorie ? getProducts(categorie.id_categorie) : [];
  $: selectedProducts = data.produits.filter(
    (e) => quantités[e.id_produit] > 0,
  );
      ...
      <div class="w-full flex flex-col gap-2 text-black">
        <div class="flex flex-col gap-1">
          {#each selectedProducts as product, i}
            <div
              class="animate-scale-up p-1 bg-red-100 rounded-3xl overflow-clip"
            >
              ...
              <div class="flex justify-center gap-1">
                <button
                  ...
                  class="w-10"><Icon icon="mdi:minus-box" /></button
                >
                <input
                  required
                  on:change={() => verifyPositivity(product.id_produit)}
                  class="text-center rounded-lg w-20"
                  type="number"
                  min="0"
                ...
                <button
                  on:click={() => increment(product.id_produit, 1)}
                  class="w-10"><Icon icon="mdi:plus-box" /></button
                >
              </div>
            </div>
          {/each}
        ...
        <div class="flex justify-center">
          <MyButton value="COMMANDER" callback={() => dialog.showModal()} />
        </div>
      </div>
    {/if}
    {#if data.categories.length > 1}
      <div class="flex gap-10">
        {#each data.categories as cat}
          <button
            class="btn text-base w-1/2 {cat == categorie
              ? 'variant-outline-surface'
              : 'variant-filled-surface'} border-2 rounded-none"
            on:click={() => (categorie = cat)}>{cat.nom}</button
          >
        {/each}
      </div>
    {/if}
    <div
      class="h-96 text-black w-full overflow-y-scroll flex flex-col bg-red-100 rounded-md"
    >
