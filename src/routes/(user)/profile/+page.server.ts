...
export const actions = {
  ...
  create_user: async ({ locals }) => {
    ...
    if (!pg) return {};
    const r = await oneSignalClient.createUser(ONESIGNAL_APP_ID, {
      identity: { id_pg: pg.id_pg.toString() },
    });
  ...
  ajouter_ancien: async ({ locals, request }) => {
    ...
    const result = z
      ...
      .refine((e) => !isNaN(e.nums) && !isNaN(e.proms))
      .safeParse(data);
    ...
    await prisma.pg.update({
      data: {
        anciens_autorises: {
          push: `${result.data.nums}ch${result.data.proms}`,
        },
      },
      where: { id_pg: locals.session.data.user?.pg.id_pg },
    });
  ...
  retirer_ancien: async ({ locals, request }) => {
    ...
    const updatedAnciens = pg.anciens_autorises.filter(
      (item) => item !== result.data.ancien,
    );
