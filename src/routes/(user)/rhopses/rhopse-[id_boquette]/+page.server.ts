...
import {
  BOQUETTES,
  BOQUETTES_LIBRE_SERVICE,
} from "$lib/server/classes/Boquette.js";
...
export type RhopseBoquette = {
  boquette: {
    id_boquette: number;
    nom: string | null;
    nom_simple: string | null;
  };
...
export const load = async ({ params }) => {
  ...
  if (!Object.values(BOQUETTES_LIBRE_SERVICE).includes(id_boquette))
    throw error(404);
...
export const actions = {
  rhopse: async ({ request, locals, params }) => {
    ...
    if (isNaN(id_boquette) || !Object.values(BOQUETTES).includes(id_boquette))
      throw error(404);
