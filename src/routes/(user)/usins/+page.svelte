<script lang="ts">
  ...
  const exportOnscrits = async () => {
    ...
    const nums = Array.from(
      new Set(
        Object.values(output).flatMap((dateObj) =>
          Object.keys(dateObj).map(Number),
        ),
      ),
    ).sort((a, b) => a - b);
    ...
    nums.forEach((num) => {
      ...
      dates.forEach((date) => {
        ...
        const value =
          output[date] && output[date][num] ? output[date][num] : "";
        rowData[date] = value;
      });
...
<div class="flex flex-col items-center gap-10">
  ...
  <div
    class="aspect-square w-full flex flex-col gap-5 items-center justify-items-center"
  >
    <div class="w-80">
        ...
        <div class="h-40 flex justify-center items-center">
          <div class="text-4xl flex flex-col items-center">
            <input
              required
              type="number"
              placeholder="nums du .onscrit"
              class="input p-2 w-80 text-center"
              bind:value={onscrit_nums}
              name="nums"
              min="1"
              max="150"
            />
