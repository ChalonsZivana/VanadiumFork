...
export const actions = {
  login: async ({ request, locals }: RequestEvent) => {
    ...
    const boq = await prisma.boquettes.findFirst({
      where: { nom_simple: uid },
      select: { nom_simple: true, mot_de_passe: true, id_boquette: true },
    });
    ...
    if (boq !== null) {
      ...
      if (a != null) {
        const update =
          locals.session.data.boquettes == undefined
            ? []
            : locals.session.data.boquettes;

        if (!update.map((e) => e.id_boquette).includes(a.id_boquette)) {
          update.push(a);
          await locals.session.update((_) => {
            return { boquettes: update };
          });
        }
        if (a.id_boquette == 20) {
  ...
  logout: async ({ locals, request }: RequestEvent) => {
    ...
    if (data.get("user")) {
      ...
      await locals.session.update((data) => {
        data.boquettes = data.boquettes.filter(
          (e) => e.id_boquette != id_boquette,
        );
        return data;
      });
    }
